...
function nav(seccion, linkEl) {
  ...
  if (seccion === 'logs')       { cargarActividad(); cargarInventarioCritico(); }
  if (seccion === 'inventario') cargarInventario();
  if (seccion === 'reportes')   iniciarReportes();
}
...
async function guardarMedico() {
  ...
  if (!document.getElementById('md-especialidad').value) {
    toast('La especialidad es obligatoria.', 'warn'); return;
  }
  if (horaInicio && horaFin && horaInicio >= horaFin) {
    toast('La hora de fin debe ser posterior a la hora de inicio.', 'warn'); return;
  }
        ...
        return `
          <tr>
            ...
            <td>
              ...
              <br/><span style="font-size:11px;color:var(--text-soft);">${esc(m.descripcion || '')}</span>
            </td>
            <td>${stockTexto}</td>
...
async function repGenerarCitas() {
  ...
  if (!fechaInicio || !fechaFin) { toast('Selecciona un rango de fechas.', 'warn'); return; }
  const params = new URLSearchParams({ fechaInicio, fechaFin });
  if (idDoctor)     params.append('idDoctor', idDoctor);
  ...
  try {
    ...
    const k = data.kpis || {};
    document.getElementById('rep-kpi-total').textContent      = k.totalCitas      ?? 0;
    document.getElementById('rep-kpi-atendidas').textContent  = k.citasAtendidas  ?? 0;
    document.getElementById('rep-kpi-pendientes').textContent = k.citasPendientes ?? 0;
    document.getElementById('rep-kpi-canceladas').textContent = k.citasCanceladas ?? 0;
    document.getElementById('rep-kpis-citas').style.display      = 'grid';
    document.getElementById('rep-tabla-citas-wrap').style.display = 'block';
    document.getElementById('rep-periodo-label').textContent = `${fechaInicio} — ${fechaFin}`;
    const detalle = Array.isArray(data.detalle) ? data.detalle : [];
    document.getElementById('rep-tbody-citas').innerHTML = detalle.length
      ? detalle.map(d => `
          <tr>
            <td>${esc(d.nombre || d.Nombres || '—')}</td>
            <td>${esc(d.Especialidad || '—')}</td>
            <td>${d.total      ?? 0}</td>
            <td>${d.atendidas  ?? 0}</td>
            <td>${d.canceladas ?? 0}</td>
            <td>${d.pendientes ?? 0}</td>
            <td>${d.total ? Math.round((d.atendidas / d.total) * 100) : 0}%</td>
          </tr>`).join('')
      : '<tr><td colspan="7" style="text-align:center;color:var(--text-soft);padding:20px;">Sin resultados para el período seleccionado.</td></tr>';
  } catch (e) {
...
function repSwitchTab(tab, btn) {
  document.getElementById('rep-panel-citas').style.display      = tab === 'citas'      ? 'block' : 'none';
  document.getElementById('rep-panel-inventario').style.display = tab === 'inventario' ? 'block' : 'none';
  document.querySelectorAll('.rep-tab').forEach(b => b.classList.remove('rep-tab--active'));
  if (btn) btn.classList.add('rep-tab--active');
}
...
function repFiltrarInv(filtro, btn) {
  ...
  let lista = _repInvData;
  if (filtro === 'alerta')  lista = _repInvData.filter(m => m.stock_actual > 0 && m.stock_actual <= m.stock_minimo);
  if (filtro === 'agotado') lista = _repInvData.filter(m => m.stock_actual === 0);
  if (filtro === 'normal')  lista = _repInvData.filter(m => m.stock_actual > m.stock_minimo);
  _repRenderInv(lista);
}
...
async function repCargarInventario() {
  try {
    ...
    const data = await res.json();
    _repInvData = Array.isArray(data.medicamentos) ? data.medicamentos : [];
    const k = data.kpis || {};
    document.getElementById('inv-kpi-total').textContent    = k.total_medicamentos ?? 0;
    document.getElementById('inv-kpi-stock').textContent    = k.en_stock           ?? 0;
    document.getElementById('inv-kpi-alerta').textContent   = k.en_alerta          ?? 0;
    document.getElementById('inv-kpi-agotados').textContent = k.agotados           ?? 0;
    _repRenderInv(_repInvData);
  } catch (e) { console.error(e); }
}

function _repRenderInv(lista) {
  document.getElementById('rep-tbody-inv').innerHTML = lista.length
    ? lista.map(m => {
        const agotado = m.stock_actual === 0;
        const alerta  = !agotado && m.stock_actual <= m.stock_minimo;
        const color   = agotado ? 'color:#c03030;font-weight:700;' : alerta ? 'color:#b07800;font-weight:700;' : 'color:var(--teal);';
        const estado  = agotado ? '🔴 Agotado' : alerta ? '⚠️ Alerta' : '✅ OK';
        return `
          <tr>
            <td><strong>${esc(m.nombre || '')}</strong></td>
            <td style="${color}">${m.stock_actual}</td>
            <td>${m.stock_minimo}</td>
            <td>${esc(m.unidad_medida || '')}</td>
            <td>$${parseFloat(m.precio_unitario || 0).toFixed(2)}</td>
...
// ══════════════════════════════════════════════
//  EXPORTAR PDF — REPORTE INVENTARIO
// ══════════════════════════════════════════════
async function exportarPDFInventario() {
  // 1. Determinar filtro activo
  const tabActiva = document.querySelector('#rep-panel-inventario .tab-btn.active');
  const filtroLabel = tabActiva ? tabActiva.textContent.trim() : 'Todos';

  // 2. Fecha y usuario
  const ahora    = new Date();
  const fechaStr = ahora.toLocaleDateString('es-SV', { weekday:'long', year:'numeric', month:'long', day:'numeric' });
  const horaStr  = ahora.toLocaleTimeString('es-SV', { hour:'2-digit', minute:'2-digit' });
  const fechaHora = `${fechaStr}, ${horaStr}`;
  const nombreUsr = usuario?.nombre || 'Administrador';

  // 3. KPIs actuales
  const kpiTotal    = document.getElementById('inv-kpi-total')?.textContent    || '—';
  const kpiStock    = document.getElementById('inv-kpi-stock')?.textContent    || '—';
  const kpiAlerta   = document.getElementById('inv-kpi-alerta')?.textContent   || '—';
  const kpiAgotados = document.getElementById('inv-kpi-agotados')?.textContent || '—';

  // 4. Armar KPIs en el PDF
  document.getElementById('pdf-fecha-generacion').textContent  = fechaHora;
  document.getElementById('pdf-usuario-generacion').textContent = nombreUsr;
  document.getElementById('pdf-filtro-label').textContent = `Filtro aplicado: ${filtroLabel}`;
  document.getElementById('pdf-footer-fecha').textContent = fechaHora;

  document.getElementById('pdf-kpis').innerHTML = `
    ${_pdfKpi('💊', kpiTotal,    'Medicamentos activos', '#2a6b5e')}
    ${_pdfKpi('✅', kpiStock,    'En stock',             '#27ae60')}
    ${_pdfKpi('⚠️', kpiAlerta,  'En alerta',            '#b07800')}
    ${_pdfKpi('🔴', kpiAgotados, 'Agotados',             '#c03030')}
  `;

  // 5. Armar tabla con los datos actuales filtrados
  const filas = [...document.querySelectorAll('#rep-tbody-inv tr')];
  document.getElementById('pdf-tbody-inv').innerHTML = filas.map((tr, i) => {
    ...
    return `<tr style="background:${bg};">
      ${celdas.map((td, ci) => {
        ...
        // limpiar emojis de estado para el PDF y poner texto limpio
        let txt = td.textContent.trim();
        return `<td style="padding:8px 12px;text-align:${align};border-bottom:1px solid #eee;">${txt}</td>`;
      }).join('')}
    </tr>`;
  }).join('');

  // 6. Mostrar contenedor y print
  const cont = document.getElementById('pdf-reporte-inv');
  cont.style.display = 'block';
  ...
  _logAuditoriaPDF(filtroLabel, fechaHora);
}

function _pdfKpi(icono, valor, label, color) {
  return `
    <div style="border:1.5px solid ${color}22;border-radius:10px;padding:14px 16px;background:${color}08;">
      <div style="font-size:20px;">${icono}</div>
      <div style="font-size:22px;font-weight:700;color:${color};margin:4px 0;">${valor}</div>
      <div style="font-size:11px;color:#666;">${label}</div>
    </div>`;
}

async function _logAuditoriaPDF(filtro, fechaHora) {
  try {
    await fetch('/api/metricas/actividad', {
      method: 'POST',
      headers: H,
      body: JSON.stringify({
        accion:      'Exportación de reporte',
        descripcion: `Reporte de inventario exportado a PDF — Filtro: ${filtro}`,
        modulo:      'Reportes',
        fecha:       new Date().toISOString(),
      })
    });
  } catch (e) {
    // Si el endpoint no existe aún, no rompe nada
    console.warn('Log auditoría PDF:', e.message);
  }
}
