...
function exportarPDFInventario() {
  ...
  if (!filas.length || document.querySelector('#rep-tbody-inv td[colspan]')) {
    toast('No hay datos de inventario para exportar.', 'warn');
    return;
  }

  // KPIs
  const kpiData = [
    { label: 'Medicamentos activos', valor: document.getElementById('inv-kpi-total')?.textContent    || '—', color: '#2a6b5e' },
    { label: 'En stock',             valor: document.getElementById('inv-kpi-stock')?.textContent    || '—', color: '#4a9e8e' },
    { label: 'En alerta',            valor: document.getElementById('inv-kpi-alerta')?.textContent   || '—', color: '#c9a84c' },
    { label: 'Agotados',             valor: document.getElementById('inv-kpi-agotados')?.textContent || '—', color: '#c0392b' },
  ];
  document.getElementById('pdf-kpis').innerHTML = kpiData.map(k => `
    <div style="background:${k.color}18;border:1.5px solid ${k.color}40;border-radius:10px;padding:12px 16px;">
      <div style="font-size:22px;font-weight:700;color:${k.color};">${k.valor}</div>
      <div style="font-size:11px;color:#666;margin-top:4px;">${k.label}</div>
    </div>`).join('');

  // Tabla
  const tbodyPDF = document.getElementById('pdf-tbody-inv');
  tbodyPDF.innerHTML = '';
  filas.forEach((tr, i) => {
    const celdas = tr.querySelectorAll('td');
    if (!celdas.length) return;
    const fila = document.createElement('tr');
    fila.style.background = i % 2 === 0 ? '#fff' : '#f7faf9';
    celdas.forEach(td => {
      const celda = document.createElement('td');
      celda.style.cssText = 'padding:8px 12px;border-bottom:1px solid #eee;';
      celda.textContent = td.textContent.trim();
      fila.appendChild(celda);
    });
    tbodyPDF.appendChild(fila);
  });

  // Fecha y usuario
  const ahora        = new Date();
  const fechaStr     = ahora.toLocaleDateString('es-SV', { day:'2-digit', month:'long', year:'numeric' });
  const horaStr      = ahora.toLocaleTimeString('es-SV', { hour:'2-digit', minute:'2-digit' });
  const fechaCompleta = `${fechaStr} ${horaStr}`;

  document.getElementById('pdf-fecha-generacion').textContent   = `Fecha: ${fechaCompleta}`;
  document.getElementById('pdf-footer-fecha').textContent       = fechaCompleta;
  document.getElementById('pdf-usuario-generacion').textContent =
    document.getElementById('usuario-nombre')?.textContent || 'Administrador';

  // Filtro activo
  const tabActiva = document.querySelector('#rep-panel-inventario .tabs .tab-btn.active');
  document.getElementById('pdf-filtro-label').textContent =
    tabActiva ? `Filtro aplicado: ${tabActiva.textContent.trim()}` : 'Mostrando: Todos los medicamentos';

  // Imprimir
  const pdfDiv = document.getElementById('pdf-reporte-inv');
