...
const PAGINAS_PERMITIDAS = new Set([
  'dashboard-admin.html', 'dashboard-medico.html', 'dashboard-paciente.html',
  'dashboard-recepcionista.html', 'agendar.html', 'forgot-password.html'
]);

app.get("/html/:pagina", (req, res) => {
  const pagina = req.params.pagina;
  if (!PAGINAS_PERMITIDAS.has(pagina)) return res.status(404).send('Not found');
...
if (process.env.SSL_KEY && process.env.SSL_CERT) {
  try {
    ...
    console.warn('[HTTPS] No se pudo cargar el certificado, levantando en HTTP:', e.message);
